...
for output in text:
    if output in "АаБбВвГгҐґДдЕеЄєЖжЗзИиІіЇїЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщЬьЮюЯя":
        if spec == 0:
            if output in 'Оо':
                transliteration = transliteration + 'jo'
                spec = 1
            else: spec = 3
        if output in 'Ьь': spec = 0
        if spec == 3: transliteration = transliteration + dict.get(output)
        if spec == 1: spec = 3
    else: transliteration = transliteration + output
